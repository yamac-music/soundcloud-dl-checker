...
- 非公開APIは利用しない
- 判定履歴はサーバーへ保存せず、端末・ブラウザ間で同期しない

## 判定履歴

判定した曲のURL、曲名、アーティスト名、アートワークURL、最新の判定結果、判定根拠、外部DLリンク、判定日時をブラウザの `localStorage` に最大100曲保存します。

同じ曲を再判定した場合は、短縮URLの解決とURLの正規化後に同一曲かを判定し、既存の履歴を最新結果で更新します。履歴は個別または一括で削除できます。ブラウザのサイトデータを削除すると履歴も削除されます。
...
履歴を含むブラウザ操作のE2Eテストを実行します。ローカル実行ではGoogle Chromeを使用します。
